...
<div class="card module-head">
    <div>
        <h2>Financiero</h2>
        <p class="muted">Cargos, pagos y estado de cobranza.</p>
    </div>
</div>

<div class="grid-2">
<div class="card"><h3>Nuevo cargo</h3><form class="stack-sm" method="POST" action="{{ route('finance.charges.store') }}">@csrf
...
<div class="card"><h3>Registrar pago</h3><form class="stack-sm" method="POST" action="{{ route('finance.payments.store') }}">@csrf
<label>Alumno</label><select name="student_id">@foreach($students as $student)<option value="{{ $student->id }}">{{ $student->full_name }}</option>@endforeach</select>
